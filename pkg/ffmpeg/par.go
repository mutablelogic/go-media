...
func NewAudioPar(samplefmt string, channellayout string, samplerate int, opts ...media.Metadata) (*Par, error) {
	...
	if samplefmt_ := ff.AVUtil_get_sample_fmt(samplefmt); samplefmt_ == ff.AV_SAMPLE_FMT_NONE {
		return nil, media.ErrBadParameter.Withf("unknown sample format %q", samplefmt)
	} else {
	...
	if err := ff.AVUtil_channel_layout_from_string(&ch, channellayout); err != nil {
		return nil, media.ErrBadParameter.Withf("channel layout %q", channellayout)
	...
	// Sample rate
	if samplerate <= 0 {
		return nil, media.ErrBadParameter.Withf("negative or zero samplerate %v", samplerate)
	} else {
...
// plus any additional options which is used for creating a stream
func NewVideoPar(pixelfmt, size string, framerate float64, opts ...media.Metadata) (*Par, error) {
	par := new(Par)
	par.SetCodecType(ff.AVMEDIA_TYPE_VIDEO)
	par.opts = opts

	// Pixel Format
	if pixfmt_ := ff.AVUtil_get_pix_fmt(pixelfmt); pixfmt_ == ff.AV_PIX_FMT_NONE {
		return nil, media.ErrBadParameter.Withf("unknown pixel format %q", pixelfmt)
	} else {
	...
	if w, h, err := ff.AVUtil_parse_video_size(size); err != nil {
		return nil, media.ErrBadParameter.Withf("size %q", size)
	} else {
		par.SetWidth(w)
		par.SetHeight(h)
	}
	...
	if framerate < 0 {
		return nil, media.ErrBadParameter.Withf("negative framerate %v", framerate)
	} else if framerate > 0 {
...
func (ctx *Par) validateAudioCodec(codec *ff.AVCodec) error {
	...
	if len(sampleformats) > 0 {
		if !slices.Contains(sampleformats, ctx.SampleFormat()) {
			return media.ErrBadParameter.Withf("unsupported sample format %v", ctx.SampleFormat())
		}
	} else if ctx.SampleFormat() == ff.AV_SAMPLE_FMT_NONE {
		return media.ErrBadParameter.With("sample format not set")
	}
	if len(samplerates) > 0 {
		if !slices.Contains(samplerates, ctx.Samplerate()) {
			return media.ErrBadParameter.Withf("unsupported samplerate %v", ctx.Samplerate())
		}
	} else if ctx.Samplerate() == 0 {
		return media.ErrBadParameter.With("samplerate not set")
	}
	if len(channellayouts) > 0 {
		valid := false
		...
		if !valid {
			return media.ErrBadParameter.Withf("unsupported channel layout %v", ctx.ChannelLayout())
		}
	} else if ctx.ChannelLayout().NumChannels() == 0 {
		return media.ErrBadParameter.With("channel layout not set")
	}
...
func (ctx *Par) validateVideoCodec(codec *ff.AVCodec) error {
	...
	if len(pixelformats) > 0 {
		if !slices.Contains(pixelformats, ctx.PixelFormat()) {
			return media.ErrBadParameter.Withf("unsupported pixel format %v", ctx.PixelFormat())
		}
	} else if ctx.PixelFormat() == ff.AV_PIX_FMT_NONE {
		return media.ErrBadParameter.With("pixel format not set")
	}
	if ctx.Width() == 0 || ctx.Height() == 0 {
		return media.ErrBadParameter.Withf("invalid width %v or height %v", ctx.Width(), ctx.Height())
	}
	if ctx.SampleAspectRatio().Num() == 0 || ctx.SampleAspectRatio().Den() == 0 {
		ctx.SetSampleAspectRatio(ff.AVUtil_rational(1, 1))
	}
	if ctx.timebase.Num() == 0 || ctx.timebase.Den() == 0 {
		return media.ErrBadParameter.With("framerate not set")
		...
		if !valid {
			return media.ErrBadParameter.Withf("unsupported framerate %v", ff.AVUtil_rational_invert(ctx.timebase))
		}
	}
