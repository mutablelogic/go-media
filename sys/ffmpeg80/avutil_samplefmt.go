...
func (v AVSampleFormat) String() string {
	if f := AVUtil_get_sample_fmt_name(v); f != "" {
		return f
	}
	return fmt.Sprintf("AVSampleFormat(%d)", int(v))
}
...
func AVUtil_next_sample_fmt(iterator *uintptr) AVSampleFormat {
	...
	// Check for end of enumeration before incrementing
	if AVSampleFormat(*iterator) >= AV_SAMPLE_FMT_NB {
		return AV_SAMPLE_FMT_NONE
	}

	// Get current sample format
	current := AVSampleFormat(*iterator)

	// Increment the iterator for next call
	*iterator += 1

	// Return the current sample format
	return current
}
