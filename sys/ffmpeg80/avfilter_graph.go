...
func (ctx *AVFilterGraph) MarshalJSON() ([]byte, error) {
	type j struct {
		Filters uint `json:"num_filters,omitempty"`
	}
	if ctx == nil {
		return json.Marshal(nil)
	}
	return json.Marshal(j{
		Filters: ctx.NumFilters(),
	})
}
...
// Dump the graph out. The graph should be configured first with AVFilterGraph_config
// for a complete dump. Returns empty string if graph cannot be dumped or is nil.
func AVFilterGraph_dump(graph *AVFilterGraph) string {
	if graph == nil {
		return ""
	}

	// Dump the graph - this works even if not configured, but output may be incomplete
