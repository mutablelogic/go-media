...
// defaults unbounded int options to INT32_MIN/INT32_MAX, so most options
// technically have min/max set even though they were never meant to be
// sliders — range size, not mere presence of min/max, decides the control.
const MAX_SLIDER_RANGE = 1000;

/**
 * Editor for an int/int64-typed codec/format Option with no `const` choices
 * (those go through enum-option instead): a slider when min/max are both
 * present and span a small enough range to drag meaningfully, otherwise a
 * plain number input. The option's default is shown alongside (and
 * restorable), and a disabled state is available for read-only contexts.
 */
export class IntOption extends LitElement {
  static properties = {
...
customElements.define("int-option", IntOption);
