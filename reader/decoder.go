...
// FrameFn is called for each frame a Decoder produces. Returning io.EOF
// stops decoding early without being treated as an error.
...
func (d *Decoder) decode(stream int, packet *ff.AVPacket) error {
	...
	for {
		f, err := frame.NewFrame(stream)
		if err != nil {
			return err
		}

		if err := ff.AVCodec_receive_frame(ctx, f.AVFrame); err != nil {
			f.Close()
