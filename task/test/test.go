...
// Main is the test main function for tests. It starts a shared task manager
// and runs the tests, providing a manager instance to each test via Begin.
func Main(m *testing.M, setup func(*manager.Manager) (func(), error), opts ...manager.Opt) {
	...
	go func() {
		runDone <- mgr.Run(runCtx, slog.Default())
	}()
	<-mgr.Ready()
